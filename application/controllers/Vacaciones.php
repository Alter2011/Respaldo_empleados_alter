...
class Vacaciones extends Base
{
    ...
    //NO14082023
    public function calendario_vacacion()
    {
        $mes = $this->input->post('mes');
        $quincena = $this->input->post('quincena_calendario');
        ...
        if ($quincena == 1) {
            ...
            if (date('Y-m-01') >= $primer_dia && $fin_dia <= date('Y-m-16')) {
            ...
            if (date('Y-m-16') >= $primer_dia && $fin_dia <= date('Y-m-d', mktime(0, 0, 0, date('m') + 1, 0, date('Y')))) {
                $data['validar_aprobar'] = 1;
            }
        }
        ...
        for ($i = 0; $i < count($empleados); $i++) {
            ...
            $mes_anio_inicio = substr($fechaInicio, 5, 5);
            $fecha_vacacion = date('Y' . "-" . $mes_anio_inicio);
            ...
            if ($anios > 0 && substr($fechaInicio, 5, 5) >= $inicio_dia && substr($fechaInicio, 5, 5) <= $ultimo_dia) {
                ...
                if (empty($verificar)) {
                    ...
                    $objeto->fecha_vacacion = $fecha_vacacion;
    ...
    public function save_vacacion()
    {
        ...
        $quincena = $verificacion[0]->fecha_aplicacion;
        $dia = new DateTime($quincena);
        $dia = $dia->format('d');
        $num_quincena = ceil($dia / 15);


        $buscar_credito = $this->Planillas_model->desembolos_creditos($id_empleado, $verificacion[0]->fecha_aplicacion, $num_quincena);
        for ($k = 0; $k < count($buscar_credito); $k++) {
         
            $ultimo_pago = $this->Planillas_model->ultimo_pago($buscar_credito[$k]->codigo);
            $numero_pagos = $this->Planillas_model->numero_pagos($verificacion[0]->id_agencia); //numero de pagos en toda la agencia (utilizado para hacer el codigo)
            $ultimo_pago = $this->Planillas_model->ultimo_pago($buscar_credito[$k]->codigo);
            ...
            while ($bandera != false) {
                ...
                if (strlen($conteo) == 1) {
                    $codigo = $verificacion[0]->id_agencia . '0' . '0' . '0' . '0' . '0' . '0' . $conteo;
                    $codigo = strtoupper($codigo);
                } else if (strlen($conteo) == 2) {
                    $codigo = $verificacion[0]->id_agencia . '0' . '0' . '0' . '0' . '0' . $conteo;
                    $codigo = strtoupper($codigo);
                } else if (strlen($conteo) == 3) {
                    $codigo = $verificacion[0]->id_agencia . '0' . '0' . '0' . '0' . $conteo;
                    $codigo = strtoupper($codigo);
                } else if (strlen($conteo) == 4) {
                    $codigo = $verificacion[0]->id_agencia . '0' . '0' . '0' . $conteo;
                    $codigo = strtoupper($codigo);
                } else if (strlen($conteo) == 5) {
                    $codigo = $verificacion[0]->id_agencia . '0' . '0' . $conteo;
                    $codigo = strtoupper($codigo);
                } else if (strlen($conteo) == 6) {
                    $codigo = $verificacion[0]->id_agencia . '0' . $conteo;
                    $codigo = strtoupper($codigo);
                } else {
                    $codigo = $verificacion[0]->id_agencia . $conteo;
                    $codigo = strtoupper($codigo);
                }
            ...
            $monto_pagar = $buscar_credito[$k]->monto_pagar - $buscar_credito[$k]->cuota_diaria;
           $buscar_credito[$k]->comprobante = $codigo;
            $interes_secofi =$buscar_credito[$k]->interes_total -$buscar_credito[$k]->interes_alter;
            $interes_alter_distr =$buscar_credito[$k]->interes_alter /$buscar_credito[$k]->interes_total;
            $interes_secofi_distr = $interes_secofi /$buscar_credito[$k]->interes_total;
            $pago_siga = 0;

            if (empty($ultimo_pago)) {
                $diferencia = date_diff(date_create($buscar_credito[$k]->fecha_desembolso), date_create($buscar_credito[$k]->fecha_aplicar));
                $total_dias = $diferencia->format('%a');
                $interes_devengado = ((($buscar_credito[$k]->monto) * ($buscar_credito[$k]->interes_total)) /$buscar_credito[$k]->dias_interes) * $total_dias;

                //$interes_pagar = ((($buscar_credito[$k]->monto)*($buscar_credito[$k]->interes_alter))/$buscar_credito[$k]->dias_interes)*$total_dias;
                //$cuota_secofi = ((($buscar_credito[$k]->monto)*($interes_secofi))/$buscar_credito[$k]->dias_interes)*$total_dias;

                if ($interes_devengado >$buscar_credito[$k]->cuota_diaria) {
                    $amortizacion_pagar = 0;
                    $abono_interes =$buscar_credito[$k]->cuota_diaria;
                    $saldo_pagar =$buscar_credito[$k]->monto;
                    $interes_pendiente = $interes_devengado -$buscar_credito[$k]->cuota_diaria;
                    //se debe dividir la cuota para poder pagar el interes
                    $monto_pagar += $interes_pendiente +$buscar_credito[$k]->cuota_diaria; //se aumenta la deuda por el interes pendiente
                    //$interes_pagar =$buscar_credito[$k]->cuota_diaria*$interes_alter_distr;
                    // $cuota_secofi =$buscar_credito[$k]->cuota_diaria*$interes_secofi_distr;
                } else {
                    $amortizacion_pagar =$buscar_credito[$k]->cuota_diaria - $interes_devengado -$buscar_credito[$k]->cuota_seguro_vida -$buscar_credito[$k]->cuota_seguro_deuda -$buscar_credito[$k]->cuota_vehicular;
                    $abono_interes = $interes_devengado;
                    $saldo_pagar =$buscar_credito[$k]->monto - $amortizacion_pagar;
                    $interes_pendiente = 0;
                }
                $pago_siga =$buscar_credito[$k]->cuota_diaria;
            } else {
                $diferencia = date_diff(date_create(substr($ultimo_pago[0]->fecha_pago, 0, 10)), date_create(date('Y-m-d')));
                $total_dias = $diferencia->format('%a');
                $interes_devengado = ((($ultimo_pago[0]->saldo) * ($buscar_credito[$k]->interes_total)) /$buscar_credito[$k]->dias_interes) * $total_dias;
               
                $all_interes = $interes_devengado + $ultimo_pago[0]->interes_pendiente;
                //$interes_pagar = ((($ultimo_pago[0]->saldo)*($$buscar_credito[$k]->interes_alter))/$$buscar_credito[$k]->dias_interes)*$total_dias;
                //$cuota_secofi = ((($ultimo_pago[0]->saldo)*($interes_secofi))/$buscar_credito[$k]->dias_interes)*$total_dias;

                if ($all_interes >$buscar_credito[$k]->cuota_diaria) {
                    $amortizacion_pagar = 0;
                    // $abono_interes =$cuota;
                    $saldo_pagar = $ultimo_pago[0]->saldo;
                    $interes_pendiente = $interes_devengado -$buscar_credito[$k]->cuota_diaria + $ultimo_pago[0]->interes_pendiente;
                    //$pago_total = $cuota;
            ...
            $pagos = array(
                ...
                'cuota_vida' =>$buscar_credito[$k]->cuota_seguro_vida,
                'cuota_deuda' =>$buscar_credito[$k]->cuota_seguro_deuda,
                'cuota_vehicular' =>$buscar_credito[$k]->cuota_vehicular,
                'credito' =>$buscar_credito[$k]->codigo,
                'puntaje_real' => 2,
                'puntaje_teorico' => 2,
    ...
    function get_empleados_vacacion()
    {
        ...
        for ($i = 0; $i < count($empleados); $i++) {
            ...
            if ($flag) {
                ...
                $empleados[$i]->disponible = 0;
                array_push($data['empleados_disponibles'], $empleados[$i]);
            }
        }

        $empleadosVacacion = $this->empleado_model->obtenerEmpleadosPorAgencia($agencia);
        for ($i = 0; $i < count($empleadosVacacion); $i++) {
            $hasVacacion = $this->Vacacion_model->verificar_vacacion($empleadosVacacion[$i]->id_contrato, $anio, $diaUno, $diaUltimo);

            if (!empty($hasVacacion)) {
                $empleadosVacacion[$i]->hasVacacion = 1;
            } else {
                $empleadosVacacion[$i]->hasVacacion = 0;
            }
        }
    ...
    public function agendar_vacacion()
    {
        ...
        for ($i = 0; $i < count($empleados); $i++) {
            ...
            if (empty($verificar)) {
                ...
                $bonos = $this->Planillas_model->bonoActual($primer_dia, $fin_dia, $empleados[$i]->id_empleado);
                // print_r($bonos);
                if ($bonos != null) {
                    for ($k = 0; $k < count($bonos); $k++) {
                        $comisiones += $bonos[$k]->cantidad;
