        <div class="col-sm-10">
            <div class="text-center well text-white blue">
                <h2>Anticipos/Herramientas Para Empleados</h2>
            </div>
            <div class="row">
                <div class="col-sm-12">
                    <div class="well">
...
<form>
    <div class="modal fade" id="Modal_Add" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <h3 class="modal-title" id="exampleModalLabel">Agregar Nuevo Anticipo/Herramienta</h3>
            ...
            <div class="form-group row">
                ...
                <div class="form-group row">
                    <label class="col-md-2 col-form-label">Tipo de Anticipo:</label>
                    <div class="col-md-10">
                        <select class="form-control" name="tipo_anticipo" id="tipo_anticipo" class="form-control">
                            <?php
                                ...
                                foreach($tipos as $a){
                            ?>
                            <option id="<?= ($tipos[$i]->id_tipo_anticipo);?>"><?php echo($tipos[$i]->nombre_tipo);?></option>
                            <?php
                                    $i++;
                                }
                            ?>
                ...
                <div class="form-group row">
                    <label class="col-md-2 col-form-label">Cantidad del anticipo:</label>
                    <div class="col-md-10">
                        <input type="text" name="cantidad_anticipo" id="cantidad_anticipo" class="form-control" placeholder="0.00">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-10">
                        <input type="date" name="fecha_anticipo" id="fecha_anticipo" class="form-control" placeholder="0.00">
    ...
    $(document).ready(function(){
                    ...
                    $('tbody').append(
                        '<tr>'+
                                ...
                                '<td style="text-align:right;">'+
                                    '<?php if ($Agregar==1) { ?><a href="#" data-toggle="modal" data-target="#Modal_Add" class="btn btn-success btn-sm item_add" data-codigo="'+registro.id_contrato+'">Agregar Anticipo</a><?php } ?>'+' '+'<?php if ($Revisar==1) { ?><a href="<?php echo base_url();?>index.php/Anticipos/verAnticipos/'+registro.id_empleado+'" class="btn btn-primary btn-sm">Revisar</a><?php } ?>'+                                    
                                '</td>'+
                                '</tr>'
                        );
                   });
        ...
        $('#show_data').on('click','.item_add',function(){ 
           ...
           $('[name="code_contrato"]').val(code);
            $('[name="cantidad_anticipo"]').val("");
            $('[name="tiempo_anticipo"]').val("");
            $('[name="fecha_anticipo"]').val("");
         ...
         $('#btn_save').on('click',function(){
            ...
            var cantidad_anticipo = $('#cantidad_anticipo').val();
            var tiempo_anticipo = $('#tiempo_anticipo').val();
            var tipo_anticipo = $('#tipo_anticipo').children(":selected").attr("id");
            var fecha_anticipo = $('#fecha_anticipo').val();
            var autorizado = $('#user').val();

                $.ajax({
                type : "POST",
                url  : "<?php echo site_url('Anticipos/insertAnticipo')?>",
                dataType : "JSON",
                data : {code:code,cantidad_anticipo:cantidad_anticipo,tipo_anticipo:tipo_anticipo,autorizado:autorizado,fecha_anticipo:fecha_anticipo},
                success: function(data){
                    console.log(data);
                    if(data == null){
                        document.getElementById('validacion').innerHTML = '';
                        document.getElementById('validacion3').innerHTML = '';
                         ...
                         $('[name="tiempo_anticipo"]').val("");
                         $('[name="fecha_anticipo"]').val("");
                         $("#tipo_anticipo option:first").attr('selected','selected'); 
                        ...
                        for (i = 0; i <= data.length-1; i++){
                            if(data[i] == 1){
                                document.getElementById('validacion').innerHTML += "Debe de Ingresar la Cantidad del Anticipo";
                            }else if(data[i] == 2){
                                document.getElementById('validacion').innerHTML += "Debe de Ingresar la Cantidad del Anticipo de Forma Correcta(0.00)";

                            }else if(data[i] == 6){
                                document.getElementById('validacion').innerHTML += "Este Monto es mayor al permitido";
