...
<form>
    <div class="modal fade" id="Modal_Add" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <h3 class="modal-title" id="exampleModalLabel">Agregar Horas Extras</h3>
            ...
            <div class="form-group row">
                    <label class="col-md-3 col-form-label">Tipo de horas extras:</label>
                    <div class="col-md-5">
                        <select class="form-control" name="tipo_horas" id="tipo_horas" class="form-control">
                            <?php
                                $i=0;
                                foreach($tipoHora as $a){
                            ?>
                            <option id="<?= ($tipoHora[$i]->id_tasa);?>"><?php echo($tipoHora[$i]->nombre);?></option>
                            <?php
                                    $i++;
                                }
                            ?>
                ...
                <div class="form-group row">
                    <label class="col-md-3 col-form-label">Cantidad de horas realizadas: </label>
                    <div class="col-md-5">
                        <input type="number" class="md-textarea form-control" id="cantidad_horas" name="cantidad_horas" min="1"> 
            ...
            <label class="col-md-3 col-form-label">fecha:</label>
                <div class="col-md-5">
                    <input type="date" class="form-control number-only" id="fecha" name="fecha">
    ...
    $(document).ready(function(){
                    ...
                    $('tbody').append(
                        '<tr>'+
                                ...
                                '<td style="text-align:right;">'+
                                    '<?php if ($Agregar==1) { ?><a href="#" data-toggle="modal" data-target="#Modal_Add" class="btn btn-success btn-sm item_add" data-codigo="'+registro.id_contrato+'">Agregar Horas</a><?php } ?>'+' '+'<?php if ($Revisar==1) { ?><a href="<?php echo base_url();?>index.php/Horas_extras/ver_horas_extras/'+registro.id_contrato+'" class="btn btn-primary btn-sm">Revisar</a><?php } ?>'+                                    
                                '</td>'+
                                '</tr>'
                        );
                   });
        ...
        $('#show_data').on('click','.item_add',function(){ 
         ...
         $('#btn_save').on('click',function(){
            ...
            var tipo_horas = $('#tipo_horas').children(":selected").attr("id");
            ...
            var agencia_prestamo = $('#agencia_prestamo').val();
                ...
                $.ajax({
                type : "POST",
                url  : "<?php echo site_url('horas_extras/crear_horas_extras')?>",
                dataType : "JSON",
                data : {code:code,tipo_horas:tipo_horas,cantidad_horas:cantidad_horas,fecha:fecha,agencia_prestamo:agencia_prestamo},
                success: function(data){
                    console.log(data);
                if ((data==null)) {
                document.getElementById('validacion').innerHTML = '';
                ...
                for (i = 0; i <= data.length-1; i++) {
                    ...
                    if(data[i] == 3){
                        ...
                        document.getElementById('validacion2').innerHTML += "Este empleado ya tiene horas en este dia";
                    }
                }
            }
                },  
