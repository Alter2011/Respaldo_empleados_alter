        <div class="col-sm-10">
            <div class="text-center well text-white blue">
                <h2>Solicitudes de Prestamos Personales</h2>
            </div>
            <div class="row">
    ...
    <div class="modal fade" id="Modal_Rechazar" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <button type="button" type="submit" id="btn_rechazar" class="btn btn-danger">Rechazar</button>
                </div>
            </div>
        </div>
    </div>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        function show_data(){
            ...
            $.ajax({
                type  : 'POST',
                url   : '<?php echo site_url('PrestamosPersonales/listarSolicitud')?>',
                dataType : 'JSON',
                ...
                success : function(data){
                   ...
                   $.each(data.solicitud,function(key, registro){
                        $('#row').append(
                            ...
                            '<center>'+
                            '<?php if ($editar==1) { ?><a id="revisar" class="btn btn-primary item_revisar"  href="<?php echo base_url();?>index.php/PrestamosPersonales/midificarSolicitud/'+registro.id_prestamo_personal+'">Editar</a><?php } ?>'+
                            ' <?php if ($aprobar==1) { ?><a id="aprobar" class="btn btn-success item_aprobar" data-codigo="'+registro.id_prestamo_personal+'">Aprobar</a><?php } ?>'+
                            ' <?php if ($rechazar==1) { ?><a id="rechazar" class="btn btn-danger item_rechazar" data-codigo="'+registro.id_prestamo_personal+'">Rechazar</a><?php } ?>'+
                            '</center>'+
                            '</div>'+
                            '</div>'+
                            '</div>' 
                   ...
                   if(data['solicitud'] == 0){
                     $('#row').append(
                            ...
                            '<div class="panel-heading">Solicitudes de Prestamos Personales</div>'+
        ...
        $('#btn_aprobar').on('click',function(){
            ...
            $.ajax({
                type : "POST",
                url  : "<?php echo site_url('PrestamosPersonales/aprobarSolicitud')?>",
                dataType : "JSON",
                data : {code:code},
                success: function(data){
        ...
        $('#btn_rechazar').on('click',function(){
            ...
            if(aceptar){
                $.ajax({
                    type : "POST",
                    url  : "<?php echo site_url('PrestamosPersonales/rechazarSolicitud')?>",
                    dataType : "JSON",
