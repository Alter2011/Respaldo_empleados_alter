...
<script type="text/javascript">
    $(document).ready(function(){
                    ...
                    $('tbody').append(
                        '<tr>'+
                            ...
                            '<td style="text-align:right;">'+
                                '<a href="#" data-toggle="modal" data-target="#Modal_Add" class="btn btn-success btn-sm item_add" data-codigo="'+registro.id_contrato+'"><span class="glyphicon glyphicon-plus-sign"></span> Agregar</a>'+' '+'<a href="<?php echo base_url();?>index.php/Faltantes/verFaltante/'+registro.id_empleado+'" class="btn btn-primary btn-sm"><span class="glyphicon glyphicon-folder-open"></span> Historial</a>'+                                    
                            '</td>'+
                        '</tr>'
                        );
                   });
        ...
        $('#btn_save').on('click',function(){
            ...
            $.ajax({
                type : "POST",
                url  : "<?php echo site_url('PermisosEmpleados/saveIncapacidad')?>",
                dataType : "JSON",
                data : {code:code,autorizante:autorizante,desde:desde,hasta:hasta,descripcion:descripcion},
                success: function(data){
                    if(data == null){
                        document.getElementById('validacion1').innerHTML = '';
                        document.getElementById('validacion2').innerHTML = '';
                        ...
                        for (i = 0; i <= data.length-1; i++){
                            if(data[i] == 1){
                                document.getElementById('validacion1').innerHTML += "Debe de ingresar desde cuando empieza la incapacidad";
                            }
                            if(data[i] == 2){
                                document.getElementById('validacion2').innerHTML += "Debe de ingresar hasta cuando termina la incapacidad";
                            }
                            if(data[i] == 3){
                                document.getElementById('validacion1').innerHTML += "La fecha desde tiene que ser menor de la fecha hasta";
                            }
                            if(data[i] == 4){
                                document.getElementById('validacion3').innerHTML += "Debe de ingresar la descripcion de la incapacidad";
                            }
                            if(data[i] == 5){
                                document.getElementById('validacion3').innerHTML += "Solo de permiten un maximo de 500 caracteres (cuentan los espacios)";
                            }     
                        }
                    }
